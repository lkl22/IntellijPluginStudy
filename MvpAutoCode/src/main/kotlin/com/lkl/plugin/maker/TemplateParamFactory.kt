    ...
    fun getParam4TemplateName(templateName: String, enterName: String, superImplName: String, contract: PsiFile?,
        ...
        when (templateName) {
            CONTRACT_TP_NAME_JAVA, CONTRACT_TP_NAME_KOTLIN, CONTRACT_TP_NO_MODEL_NAME_JAVA, CONTRACT_TP_NO_MODEL_NAME_KOTLIN -> {
                ...
                liveTemplateParam["VN"] = superVNameNoGeneric?.substring(superVNameNoGeneric.lastIndexOf('.') + 1 until superVNameNoGeneric.length)
                        ?: "IView"
                liveTemplateParam["MN"] = superMNameNoGeneric?.substring(superMNameNoGeneric.lastIndexOf('.') + 1 until superMNameNoGeneric.length)
                        ?: "IModel"
                liveTemplateParam["PN"] = superPNameNoGeneric?.substring(superPNameNoGeneric.lastIndexOf('.') + 1 until superPNameNoGeneric.length)
                        ?: "IPresenter"
